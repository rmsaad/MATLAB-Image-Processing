 ...
 *      cutoff Threshold
 */
void im_threshold(int width, int height, const double *input, double *output, double T, double phi){
   for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++)
        {
           ...
           if(input[ind] >= T){
                ...
                output[ind] = 1 + tanh(phi * (input[ind] - T));
           }
        } 
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    if (mxIsDouble(img))
    {
        ...
        im_threshold(width, height, img_ptr, out_ptr, *T, *phi);
    }   
}